...
typedef struct sim{ 
    ...
    void init(){
        ...
        state first = {sx,sy,0,0,h(sx,sy,tx,ty,wmin,type,mul)};
        first.f = first.h;
        pq.push(first); 
        visited[sx][sy] = true;
        p[sx][sy] = {sx,sy};
...
void BFS(sim &s){
    ...
    for (int i = 0 ; i < 4 ; i++){
        ...
        int ny = y + dy[i]; 
        if (valid(s,nx,ny)){
            state next = {nx,ny,cur.d+1,cur.g+s.w[s.M[nx][ny]]};
...
void GBFS(sim &s){
    ...
    for (int i = 0 ; i < 4 ; i++){
        ...
        int ny = y + dy[i]; 
        if (valid(s,nx,ny)){
            state next = {nx,ny,cur.d+1,cur.g+s.w[s.M[nx][ny]],h(nx,ny,s.tx,s.ty,s.wmin,s.type,s.mul)};
            next.f = next.h;
            s.visited[nx][ny] = true; s.p[nx][ny] = {x,y};
            s.pq.push(next); 
        }
